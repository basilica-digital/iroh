...
mod tests {
    ...
    /// Regression test: an empty `insert_multiple` must not drain pending
    /// resolve requests.
    ///
    /// This happens when multiple concurrent `connect_with_opts` calls send
    /// `ResolveRemote` messages with empty addrs. The first push a tx, then
    /// the second's `insert_multiple([])` used to drain that tx with
    /// `NoResults { errors: [] }` — even though an address lookup was still
    /// in flight and would shortly have resolved it.
    #[test]
    ...
    /// `address_lookup_finished(Ok(()))` with no paths still emits
    /// `NoResults` — the real "lookup done but nothing was found" signal must
    /// reach callers.
    #[test]
